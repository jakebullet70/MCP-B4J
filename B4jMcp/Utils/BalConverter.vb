...
Namespace Utils
    ''' <summary>
    ''' Port of the official B4X BalConverter.bas (B4J) to VB.NET.
    ''' Converts B4X binary layout files (.bjl for B4J; same B4XSerializator format as .bal/.bil) to/from JSON.
    ...
    Public Class BalConverter
        ...
        ' When True, TYPE_NULL/TYPE_RECT entries are stripped on write (B4i .bil variant).
        ' B4J always uses the full format, so B4jMcp constructs this with False.
        Private ReadOnly _stripNullRect As Boolean

        Public Sub New(stripNullRect As Boolean)
            _stripNullRect = stripNullRect
        End Sub
        ...
        Public Function ConvertBalToJsonInMemory(stream As Stream) As JObject
            Using reader = New BinaryReader(stream, Encoding.UTF8, leaveOpen:=True)
                ...
                If version < 3 Then
                    Throw New InvalidDataException($"Unsupported layout version: {version}")
                End If
        ...
        Private Sub WriteMap(writer As BinaryWriter, m As JObject, cache As Dictionary(Of String, Integer))
            For Each prop As KeyValuePair(Of String, JToken) In m
                ...
                ' When stripping (B4i .bil), skip TYPE_NULL and TYPE_RECT entries
                If _stripNullRect AndAlso val IsNot Nothing AndAlso val.Type = JTokenType.Object Then
